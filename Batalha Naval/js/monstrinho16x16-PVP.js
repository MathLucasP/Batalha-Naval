 ...
 */
 ...
 /* Array com a descrição dos tipos de navios que os jogadores podem posicionar no tabuleiro.
    ...
    Exemplo:
    - shiptypes[0]: ["Rebocador", 2, 4], representa um "Rebocador" de tamanho 2, com 5 unidades disponíveis.
 */
 var shiptypes = [
    Rebocador = ["Rebocador", 2, 5], // Navio de tamanho 2, 5 unidades
    Contratopedeiro = ["Contratopedeiro", 3, 4], // Navio de tamanho 3, 4 unidades
    Cruzador = ["Cruzador", 4, 3], // Navio de tamanho 4, 3 unidades
    Porta_Avioes = ["Porta-avioes", 5, 2] // Navio de tamanho 5, 2 unidade
 ];
...
/* Função responsável por configurar a grade de navios para um jogador (Player 1 ou Player 2).
   ...
   function setupPlayer(isPlayer1) {
    ...
    for (tipoNavio = shiptypes.length - 1; tipoNavio >= 0; --tipoNavio) {  // Começa com o último navio (maior) e vai até o primeiro (menor).
        var i;
        ...
        for (i = 0; i < shiptypes[tipoNavio][2]; ++i) {
            // Gera aleatoriamente a orientação do navio:
            //   - 0 significa horizontal.
            //   - 1 significa vertical.
            var orientacao = Math.floor(Math.random() * 2);  // Gera 0 ou 1 aleatoriamente.
 
            // Define o comprimento do navio e ajusta os limites da grade dependendo da orientação.
            var comprimentoNavio = shiptypes[tipoNavio][1],  // Obtém o comprimento do navio atual.
                limiteX = gridx, limiteY = gridy,  // Define os limites da grade inicialmente como a largura e altura totais.
                direcaoX = 0, direcaoY = 0;  // Variáveis que indicam a direção em que o navio será posicionado (x ou y).
 
            if (orientacao == 0) {  // Se a orientação for horizontal:
                limiteX = gridx - comprimentoNavio;  // Limita a largura para que o navio caiba na linha sem exceder a grade.
                direcaoX = 1;  // Incrementa x para preencher as células na horizontal.
            } else {  // Se a orientação for vertical:
                limiteY = gridy - comprimentoNavio;  // Limita a altura para que o navio caiba na coluna sem exceder a grade.
                direcaoY = 1;  // Incrementa y para preencher as células na vertical.
            }
            ...
            do {
                ...
                coordenadaY = Math.floor(Math.random() * limiteY);  // Escolhe uma posição aleatória na vertical dentro do limite permitido.
                coordenadaX = Math.floor(Math.random() * limiteX);  // Escolhe uma posição aleatória na vertical dentro do limite permitido.
 
                // Variáveis auxiliares para verificar a ocupação das células.
                var j, cx = coordenadaX, cy = coordenadaY;
                ...
                for (j = 0; j < comprimentoNavio; ++j) {
                    ...
                    cx += direcaoX;  // Se horizontal, move-se para a próxima coluna.
                    cy += direcaoY;  // Se vertical, move-se para a próxima linha.
                }
            ...
            var j, cx = coordenadaX, cy = coordenadaY;
            for (j = 0; j < comprimentoNavio; ++j) {
                grid[cy][cx][0] = ship[orientacao][tipoNavio][j];  // Preenche a célula com o ID do navio.
                grid[cy][cx][1] = shipNo;  // Armazena o número do navio na célula.
                grid[cy][cx][2] = dead[orientacao][tipoNavio][j];  // Armazena o estado inicial (não afundado) do navio.
                cx += direcaoX;  // Avança para a próxima célula na direção x (se horizontal).
                cy += direcaoY;  // Avança para a próxima célula na direção y (se vertical).
            }
 
            /* Atualiza as informações de navios e vidas do jogador. */
            if (isPlayer1) {
                // Se for o jogador 1:
                player1Ships[shipNo] = [tipoNavio, shiptypes[tipoNavio][1]];  // Armazena o tipo e comprimento do navio.
                player1Lives++;  // Incrementa o número de vidas (ou navios ativos) do jogador 1.
            } else {
                // Se for o jogador 2:
                player2Ships[shipNo] = [tipoNavio, shiptypes[tipoNavio][1]];  // Armazena o tipo e comprimento do navio.
